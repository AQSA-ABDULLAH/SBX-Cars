...
const Navbar = () => {
  ...
  const toggleMobileMenu = () => {
    ...
    setIsChecked(!isChecked); // Toggle the checked state when clicking the mobile menu
  };
  ...
  return (
    <nav className="font-[16px] px-[16px] py-[20px] grid grid-cols-[repeat(3,_minmax(0,_1fr))] gap-4 justify-center items-center relative w-full transition-all duration-300">
      ...
      <div
        ...
        onClick={toggleMobileMenu} // Trigger the toggle function
      >
        <input
          type="checkbox"
          ...
          className="hidden"
          checked={isChecked}
          onChange={() => setIsChecked(!isChecked)} // This keeps the state in sync
        />
        <span
